...
-- changeset WKSP_QCAADEMO23AI:1766158210446 stripComments:false  logicalFilePath:release_base_release_1.0\wksp_qcaademo23ai\package_bodies\eba_demo_ir_pkg.sql
-- sqlcl_snapshot src/database/wksp_qcaademo23ai/package_bodies/eba_demo_ir_pkg.sql:null:ad4ec65ef3afb30880b4496f227466e9880176de:create
...
create or replace package body eba_demo_ir_pkg as
    ...
    begin
        ...
        ------------------------------------------------------------------------
        if l_start > 0 then
            return substr(l_link,
                          l_start + 13,
                          instr(l_link, ':', l_start + 13) - l_start - 13);
        else
            return null;
        end if;
    ...
    is
        ...
        select
            ...
            breadcrumb_entry_id,
            substr(
                sys_connect_by_path(defined_for_page, ':'),
                1,
                instr(
                    sys_connect_by_path(defined_for_page, ':'),
                    ':',
        ...
        for bc_rec in bc_csr loop
            ...
            for x in 1..l_pages.count loop
                if l_pages(x) is not null then
                    select
                        bc_rec.defined_for_page,
                        bc_rec.breadcrumb_entry_id,
                        pg.page_id
                    into l_pageref
                    from
                        apex_application_pages pg
                    where
                            pg.application_id = p_app_id
                        and ( to_char(pg.page_id) = l_pages(x)
                              or pg.page_alias = l_pages(x) );
    ...
    is
        ...
        select
            ...
            count(*) c
        from
            table ( getbreadpagerefstable(l_app) ) refs
        group by
            target_page_id;
    ...
    begin
        ...
        for pg_rec in pg_csr loop
            ...
            select
                count(*)
            into l_report.list_link_count
            from
                apex_application_list_entries lst
            where
                    lst.application_id = l_app
                and regexp_like ( lst.entry_target,
                                  l_regexp );
            -- Branches
            select
                count(*)
            into l_report.branch_count
            from
                apex_application_page_branches brn
            where
                    brn.application_id = l_app
                and brn.page_id <> pg_rec.page_id
                and regexp_like ( brn.branch_action,
                                  l_regexp );
            -- Buttons
            select
                count(*)
            into l_report.button_count
            from
                apex_application_page_buttons btn
            where
                    btn.application_id = l_app
                and btn.page_id <> pg_rec.page_id
                and regexp_like ( btn.redirect_url,
                                  l_regexp );
            -- Breadcrumbs
            ...
            select
                count(*)
            into l_report.reportcol_count
            from
                apex_application_page_rpt_cols rc
            where
                    rc.application_id = l_app
                and rc.page_id <> pg_rec.page_id
                and regexp_like ( rc.column_link_url,
                                  l_regexp );
            -- Report Columns (interactive)
            select
                count(*)
            into l_report.irepcol_count
            from
                apex_application_page_ir_col rc
            where
                    rc.application_id = l_app
                and rc.page_id <> pg_rec.page_id
                and regexp_like ( rc.column_link,
                                  l_regexp );
            -- Interactive Report detail links
            select
                count(*)
            into l_report.ireplink_count
            from
                apex_application_page_ir api
            where
                    api.application_id = l_app
                and api.page_id <> pg_rec.page_id
                and pg_rec.page_id = getpageidnum(api.application_id, api.detail_link_target);
            -- Tabs
            select
                count(*)
            into l_report.tab_count
            from
                apex_application_tabs tb
            where
                    tb.application_id = l_app
                and tb.tab_page = pg_rec.page_id;
            --
            l_report.total := l_report.list_link_count + l_report.branch_count + l_report.button_count + l_report.breadcrumb_count + l_report.reportcol_count
            + l_report.irepcol_count + l_report.ireplink_count + l_report.tab_count;
